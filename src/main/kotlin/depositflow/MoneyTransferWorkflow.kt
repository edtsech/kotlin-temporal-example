...
class MoneyTransferWorkflow: IMoneyTransferWorkflow {
    ...
    private val perActivityMethodOptions: Map<String, ActivityOptions> = object : HashMap<String, ActivityOptions>() {
        init {
            put(WITHDRAW, ActivityOptions.newBuilder().setHeartbeatTimeout(Duration.ofSeconds(5)).build())
        }
    }

    private val account: IAccountActivity =
        Workflow.newActivityStub(IAccountActivity::class.java, defaultActivityOptions, perActivityMethodOptions)
